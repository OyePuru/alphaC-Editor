...
#define alphaC_VERSION "0.0.1"
#define alphaC_TAB_STOP 8
// data
    ...
    if (filerow >= E.numrows) {
      if (E.numrows == 0 && y == E.screenrows / 3) {
        ...
        int welcomelen = snprintf(welcome, sizeof(welcome),
          "alphaC editor -- VERSION %s", alphaC_VERSION);
...
void editorUpdateRow(erow *row) {
  ...
  for (j = 0; j < row->size; j++)
    if (row->chars[j] == '\t') tabs++;
  free(row->render);
  row->render = malloc(row->size + tabs*(alphaC_TAB_STOP - 1) + 1);
  int idx = 0;
  for (j = 0; j < row->size; j++) {
    if (row->chars[j] == '\t') {
      row->render[idx++] = ' ';
      while (idx % alphaC_TAB_STOP != 0) row->render[idx++] = ' ';
    } else {
